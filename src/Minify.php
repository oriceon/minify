...
use Oriceon\Minify\Exceptions\InvalidArgumentException;
use Oriceon\Minify\Providers\JavaScript;
use Oriceon\Minify\Providers\StyleSheet;
...
class Minify
{
    ...
    */
    private function assetDirHelper($ext, $dir)
    {
        ...
        if (count($files) > 0)
        {
            if ($this->config['reverse_sort']) rsort($files);
            else sort($files);
    ...
    */
    private function process($file)
    {
        ...
        if
        (
            $this->minifyForCurrentEnvironment() &&
